package jt_test_bed::jt_test_bedClient;
...
sub new
{
    ...
    my $self = {
	client => jt_test_bed::jt_test_bedClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "jt_test_bed.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for jt_test_bed::jt_test_bedClient\n";
    }
    if ($sMajor == 0) {
        warn "jt_test_bed::jt_test_bedClient version is $svr_version. API subject to change.\n";
    }
}
...
package jt_test_bed::jt_test_bedClient::RpcClient;
