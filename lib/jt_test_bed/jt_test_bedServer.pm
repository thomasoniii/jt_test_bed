package jt_test_bed::jt_test_bedServer;
...
sub call_method {
    ...
    my $ctx = jt_test_bed::jt_test_bedServerContext->new($self->{loggers}->{userlog},
...
{
    # Service jt_test_bed requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for jt_test_bed but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = jt_test_bed::jt_test_bedServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service jt_test_bed.");
    }
...
package jt_test_bed::jt_test_bedServerContext;

use strict;

=head1 NAME

jt_test_bed::jt_test_bedServerContext
...
package jt_test_bed::jt_test_bedServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = jt_test_bed::jt_test_bedServer->new(
